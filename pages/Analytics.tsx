...
import { Download, Activity, TrendingUp, ArrowUpRight } from 'lucide-react';
import { Vehicle } from '../types';

const funnelData = [
  { name: 'Anúncios', value: 100 },
  { name: 'Vistas', value: 85 },
  { name: 'Interesse', value: 32 },
...
const Analytics: React.FC<AnalyticsProps> = ({ vehicles }) => {
  // Lógica de Analytics Real: Mix de Estoque
    ...
    <div className="space-y-10 animate-in fade-in duration-1000">
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
        <div className="space-y-2">
          ...
          <p className="text-slate-500 text-sm md:text-base font-medium ml-5 md:ml-6">Métricas de conversão e tendências do pátio.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white p-10 rounded-[48px] border border-slate-100 shadow-sm">
           <div className="flex items-center justify-between mb-12">
             <div>
               <h3 className="text-[9px] font-black text-orange-600 uppercase tracking-[0.2em] mb-1">Fluxo de Venda</h3>
               <p className="text-xl font-black text-slate-900 tracking-tighter leading-none">Funil de Conversão</p>
             </div>
             <span className="text-[10px] font-black text-emerald-600 bg-emerald-50 px-4 py-2 rounded-xl border border-emerald-100">Conv. Final: 6%</span>
           </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="bg-slate-900 p-10 rounded-[48px] text-white relative overflow-hidden group">
           ...
           <h4 className="text-lg font-black tracking-tight mb-4">Giro {vehicles.length > 5 ? '15%' : '8%'} acima da média</h4>
           <p className="text-xs text-slate-400 font-medium leading-relaxed">Com base no seu estoque, veículos de ticket médio são sua maior força.</p>
        </div>
        ...
        <div className="bg-white p-10 rounded-[48px] border border-slate-100 shadow-sm flex flex-col justify-between">
           ...
           <div className="flex items-center gap-2 text-emerald-600 text-[9px] font-black uppercase mt-6">
             <ArrowUpRight className="w-3.5 h-3.5" /> Estável este mês
           </div>
        </div>
        ...
        <div className="bg-white p-10 rounded-[48px] border border-slate-100 shadow-sm flex flex-col justify-between">
           <div>
             ...
             <h4 className="text-2xl font-black text-slate-900 tracking-tighter">{(funnelData[2].value / funnelData[0].value * 10).toFixed(1)}</h4>
           </div>
           <p className="text-slate-300 text-[9px] font-black uppercase mt-6">Ritmo de interesse</p>
        </div>
      </div>
    </div>
  );
